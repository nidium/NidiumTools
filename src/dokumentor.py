...
class BasicDoc( object ):
	...
	def to_dict( self ):
		...
		return {}
	def check_type( self, var, var_name, base_type ):
		"""
		Wrapper for isinstance that throws an error message
		>>> a = BasicDoc()
		>>> a.check_type( 1, "h", int )
		>>> a.check_type( 1, "h", str )
	...
	def check_list_type(self, list_name, var_name, base_class_type ):
		"""Forces the variable into a ( empty ) list. If it was a list, all items must be of the correct type.
		>>> dummy = BasicDoc()
		>>> a = [1, 2 ]
		>>> b = dummy.check_list_type( a, "i", int )
		>>> a == b
		True
		
		>>> b = dummy.check_list_type( None, "i", int )
		>>> b == []
		True
		
		>>> c = dummy.check_list_type( [], "i", int )
		>>> c == []
		True
		
		>>> d = dummy.check_list_type( 1, "i", int )
		>>> d[0] == 1
		True
		
		>>> e = dummy.check_list_type( ['', 1] , "i", int )
		Traceback (most recent call last):
		 ...
		...
		if type( list_name).__name__ == 'ObjectDoc':
			...
			self.check_type( list_name, var_name, list )
			for item in list_name:
				self.check_type( item, var_name, base_class_type )
		return list_name
	@staticmethod
	...
	def to_markdown( self ):
		...
		for typed in self.typed:
			if (type(self.typed).__name__) == 'ObjectDoc':
				types.append(self.typed.to_markdown())
			else:
				types.append(self.typed.get())
		lines += self.name.get() + "\t'" + "', '".join( typed ) + "'\t" + self.is_readonly.get() + "\t" + self.description.get()
		return lines
	def to_dict( self ):
		"""Prepare a normal interface to export data."""
		data = super( self.__class__, self ).to_dict( )
		...
		for types in self.types:
			if (type(self.typed).__name__) == 'ObjectDoc':
				data['typed'].append( self.typed.to_dict() );
			else:
				data['typed'].append(self.typed)
		return( data )
...
"""
	...
	def to_markdown( self ):
		...
		if (type(self.typed).__name__) == 'ObjectDoc':
			...
			typed = self.typed
		lines += "'" +  "', '".join( typed ) + "'\t" + self.description.get()
		return lines
	def to_dict( self ):
		"""Prepare a normal interface to export data."""
		data = super( self.__class__, self ).to_dict( )
	...
	def to_markdown( self ):
		...
		for typed in self.typed:
			if (type(typed).__name__) == 'ObjectDoc':
				...
				types.append( self.typed)
		lines += self.name.get() + "\t'" + "', '".join( types ) + "'\t" + is_optional.get() + "\t" + self.description.get()
		return lines
	def to_dict( self ):
		"""Prepare a normal interface to export data."""
...
class ObjectDoc( BasicDoc ):
	...
	def __init__( self, obj ):
		...
		super( self.__class__, self ).__init__( )
		self.check_list_type( obj, "obj", list )
		self.data = []
